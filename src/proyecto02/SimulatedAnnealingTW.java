 ...
 */
public class SimulatedAnnealingTW {
     ...
     */
    public void setMatrizTimes(String[][] matrizTimes) {
    ...
    public void iniciaSistema(){
        FechaMaster.set(Calendar.DAY_OF_MONTH,17);
        FechaMaster.set(Calendar.HOUR_OF_DAY,2);
        FechaMaster.set(Calendar.MINUTE,30);
    ...
    private static void verificaTiempo(SolutionTW sol, String Inicio, String Final, int Minutes, int index){
        String[] arregloInicio = Inicio.split(":");
        String[] arregloFinal = Final.split(":");
        
        int[] timeInicio = new int[arregloInicio.length];
        int[] timeFinal = new int[arregloFinal.length];        
        
        for(int i = 0; i < timeInicio.length; i++){
            timeInicio[i] = Integer.parseInt(arregloInicio[i]);
        }

        for(int i = 0; i < timeFinal.length; i++){
            timeFinal[i] = Integer.parseInt(arregloFinal[i]);
        }
    ...
    public static boolean afterTempo(int[] tempo, int[] time){
        if(tempo[1] < time[0]){
            return true;
        }else{
            if(tempo[1] > time[0]){
    ...
    public static boolean beforeTempo(int[] tempo, int[] time){
        if(tempo[1] > time[0]){
            return true;
        }else{
            if(tempo[1] < time[0]){
    ...
    public static boolean compareCalendar(Calendar inicio, Calendar fin){
        ...
        for(int i = 0; i < s.getSol().length; i++){
            ...
            if(it.hasNext()){
                ...
                while(it.hasNext()){
                    ...
                    String Inicio = matrizTimes[p2][0];
                    String Final = matrizTimes[p2][1];
    ...
    */
    ...
    public void genFiles(){
        File dirg = new File("report/report-TW" + ins.length + numCam + capacidad);
        dirg.mkdirs();
        String t = "report/report-TW" + ins.length + numCam + capacidad;
